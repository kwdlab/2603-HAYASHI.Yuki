...
Docker（veth/bridge 経路・VM・ホスト直結などの実行形態）や、OS（例: Ubuntu/Linux・FreeBSD）を変えながら iperf3 を繰り返し実行し、Throughput・Retr（TCP再送）を同一形式で記録することで、環境差によるスループット低下と再送増加の傾向を定量的に比較できるようにしています。

加えて、MTU拡張（例: 1500→64000）や replay_window の調整など、設定条件を変更した場合の効果も同じ指標で検証しています。
